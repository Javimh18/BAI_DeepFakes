...
class TripleTrainLoader(Dataset):
    def __init__(self, paths_anchors, labels_anchors, paths_positives, labels_positives, paths_negatives, labels_negatives, face_detector, transform, feature_extractor):
        self.paths_anchors = paths_anchors
        self.labels_anchors = labels_anchors
        self.paths_positives = paths_positives
        self.labels_positives = labels_positives
        self.paths_negatives = paths_negatives
        self.labels_negatives = labels_negatives
        self.transform = transform
        self.face_detector = face_detector
        self.feature_extractor = feature_extractor

    def __len__(self):
        return len(self.paths_anchors)

    def __getitem__(self, item):

        anchor = Image.open(os.path.join(DATASET_PATH, self.paths_anchors[item]))
        positive = Image.open(os.path.join(DATASET_PATH, self.paths_positives[item]))
        negative = Image.open(os.path.join(DATASET_PATH, self.paths_negatives[item]))

        anchor_face = self.__face_detection__(anchor)
        positive_face = self.__face_detection__(positive)
        negative_face = self.__face_detection__(negative)

        anchor_label = self.labels_anchors[item] 
        positive_label = self.labels_positives[item] 
        negative_label = self.labels_negatives[item] 

        return anchor_face, anchor_label, positive_face, positive_label, negative_face, negative_label

    def __face_detection__(self, image):
        margin = 25
