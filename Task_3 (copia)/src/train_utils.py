...
def train_model(model, train_loader, val_loader, test_loader, optimizer, criterion, num_epochs, device, path, best):
    ...
    for epoch in range(num_epochs):
        ...
        if val_accuracy >= best_val_accuracy and best == True:
            best_val_accuracy = val_accuracy
            torch.save(model.state_dict(), os.path.join(path, 'best_in_val.pth'))
...
def validate_model(best_model, val_loader, criterion, device, path):
    ...
    auc_best_val, _, _ = computeROC(list_labels, list_outputs, os.path.join(path, "valROC"))
...
def test_model(best_model, test_loader, criterion, device):
    ...
    print(hits, len(test_loader.dataset))
...
def train_siamese_model(model, train_loader, optimizer, triplet, num_epochs, device, model_name, path):
    min_loss = 1
    list_loss_train = []
    model.to(device)
    for epoch in range(num_epochs):
        ...
        for anchor, anchor_label, positive, positive_label, negative, negative_label in train_loader:
            anchor, anchor_label, positive, positive_label, negative, negative_label  = anchor.to(device), anchor_label.to(device), positive.to(device), positive_label.to(device), negative.to(device), negative_label.to(device)
            
            optimizer.zero_grad()

            embbeding_anchors = model(anchor)
            embbeding_positive = model(positive)
            embbeding_negative = model(negative)

            loss_triplet = triplet(embbeding_anchors, embbeding_positive, embbeding_negative)
            loss_triplet.backward()
            optimizer.step()

            train_loss += loss_triplet.item() * anchor.size(0)
