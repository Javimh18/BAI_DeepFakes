...
class VAE(nn.Module):
    ...
    image processing. It will take as input the image features and it will
    reproduce them at the output by sampling from a latent space.
    
    :param input_size: (n_channels, H, W) The dimensions of the image to be processed
    ...
    :param is_featExtract: Outputs z and mu if true, else z from the reparametri-
                            zation trick.
    '''
    def __init__(self, input_size=(1,256, 64), # C,n_freq_bins,n_frames
                 latent_dim=128, 
                 n_convLayers=3, 
        ...
        self.channels_in, _, _ = input_size
    ...
    def _enc_conv_layers(n_layer=3, n_channel=[1,8,16,32], filter_size=[1,3,3], stride=[1,2,2]):
        ...
        ment a 2D conv to process images.
        :param n_layer: number of conv. layers
        :param n_channel: in/out number of channels for each layer. 
        ...
        enc_conv_layers = nn.Sequential(*enc_conv_layers)
        
        # initializing parameters
        for _, param in enc_conv_layers.named_parameters():
            if isinstance(param, nn.Conv2d):
                nn.init.kaiming_normal_(param, nonlinearity='relu')
            
        return enc_conv_layers
    ...
    def _dec_deconv_layers(n_layer=3, n_channel=[32,16,8,1], filter_size=[3,3,1], stride=[2,2,1]):
        ...
        we will make use of the deconv 2-D layers, inflating the spatial resolution.
        ...
        dec_deconv_layers = nn.Sequential(*dec_deconv_layers)

        # initializing parameters
        for _, param in dec_deconv_layers.named_parameters():
            if isinstance(param, nn.ConvTranspose2d):
                nn.init.kaiming_normal_(param, nonlinearity='relu')
            
        return dec_deconv_layers
    ...
    def _fully_connected(n_layer, hidden_dim):
        ...
        # initializing parameters
        for _, param in fc_layers.named_parameters():
